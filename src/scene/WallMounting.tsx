...
export function WallMounting() {
  ...
  // Back Wall (Z = -depth/2) for wall1 and wall2
  ...
  // Parallel Opposed Walls for wall_opposed (Left & Right walls spanning full depth + margin)
  const opposedWallDepth = depth + extraMargin
  const opposedLeftX = -width / 2 - wallThickness / 2
  const opposedRightX = width / 2 + wallThickness / 2

  return (
    <group>
      {/* ================= REAR WALL (wall1 & wall2) ================= */}
      {(mounting === 'wall1' || mounting === 'wall2') && (
      ...
      {/* ================= TWO OPPOSED PARALLEL WALLS (wall_opposed) ================= */}
      {mounting === 'wall_opposed' && (
        <>
          {/* Left Opposed Wall */}
          <group position={[opposedLeftX, wallHeight / 2, 0]}>
            <mesh castShadow receiveShadow material={wallMat}>
              <boxGeometry args={[wallThickness, wallHeight, opposedWallDepth]} />
            </mesh>
            <mesh position={[0, wallHeight / 2 + 0.02, 0]} castShadow receiveShadow material={trimMat}>
              <boxGeometry args={[wallThickness + 0.04, 0.04, opposedWallDepth + 0.04]} />
            </mesh>
            <mesh position={[wallThickness / 2 + 0.01, -wallHeight / 2 + 0.04, 0]} castShadow receiveShadow material={trimMat}>
              <boxGeometry args={[0.02, 0.08, opposedWallDepth]} />
            </mesh>
          </group>

          {/* Right Opposed Wall */}
          <group position={[opposedRightX, wallHeight / 2, 0]}>
            <mesh castShadow receiveShadow material={wallMat}>
              <boxGeometry args={[wallThickness, wallHeight, opposedWallDepth]} />
            </mesh>
            <mesh position={[0, wallHeight / 2 + 0.02, 0]} castShadow receiveShadow material={trimMat}>
